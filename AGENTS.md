...
- `external-dns` -> router `/etc/init.d/external-dns`
- `tiktok-custom-dns` -> router `/etc/init.d/tiktok-custom-dns`, **diverged**: the router copy (md5 `cf2cbb9e9e5849dee880beef9dc83f7e`) sends TikTok domains to the local `stubby` on `127.0.0.1#5054` and skips comment lines, the repo copy (md5 `d3ae3f5599d12925099cfb80a82028bf`) sends them to the public resolvers `84.21.189.133`/`193.23.209.189`. Do not deploy this file as-is; see `REFERENCE_MAP.md`
...
- Discord, voice included, goes to the `WARP` group through the `warp` rule set: it is blocked *by* Russia like Twitter and Meta, which is what that set means, while `vpn` means services that geo-block Russia. `vpn` is also wrong operationally: it now leads with a `url-test` ranked by HTTP latency, which says nothing about UDP, and it falls back to a relayed WARP hop instead of degrading in place. Voice is a separate UDP/RTP connection to a hostname the gateway hands out at runtime under `discord.media`, so fake-IP applies and the existing `198.18.0.0/16` TPROXY rule already matches `udp`: no firewall or `config.yaml` change is needed. Discord and Telegram share one path: the `WARP` group's `awg0` → `awg1` → `DIRECT` fallback. Measured 2026-09-10 by real IP, `discord.com/api/v9/gateway` answered 200 over `awg0`, `awg1` and `awg2` alike, so dropping `awg2` costs Discord nothing.
