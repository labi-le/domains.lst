...
- The syslog ring buffer has to outlive an incident to be worth anything. At the stock 64 KiB a single LAN torrent client retrying a dead peer through `RULE-SET,warp_ip` fills it with `dial WARP ... connection refused` warnings and leaves 17 seconds of history, which cannot diagnose anything intermittent. Raised to 512 KiB (UCI `system.@system[0].log_size`).
